...
class MainActivity : AppCompatActivity() {
    ...
    private val incrementBroadcastReceive = object: BroadcastReceiver() {
    ...
    override fun onStart() {
        super.onStart()
        // Check the versoin to make it compatible with my version
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            registerReceiver(incrementBroadcastReceive, IntentFilter("INCREMENT_VALUE_ACTION"), RECEIVER_EXPORTED)
        }else {
            registerReceiver(incrementBroadcastReceive, IntentFilter("INCREMENT_VALUE_ACTION"))
        }
    }

    override fun onStop() {
        super.onStop()
        unregisterReceiver(incrementBroadcastReceive)
    }
